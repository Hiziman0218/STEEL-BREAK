namespace RaycastPro
{
    ...
    public abstract class BaseDetector : RaycastCore
    {
        ...
        protected const string TIgnore = "このソルバーは非常に高速に動作し、検出ポイントを確認する際の不要なパフォーマンス消費を回避します。";
        protected const string TNearest = "このソルバーは、コライダーの最も近い点を SolverPoint に対して算出し、それをエリア内の判定および視線検出の基準とします。";
        protected const string TPivot = "Pivot は Ignore と同じくらい高速に動作します。このオプションは、速度と検出ポイントの両方を同時に確保したい場合に非常に便利です。";
        protected const string TCenter = "コライダーの境界中心に検出ポイントを設定します。";
        protected const string TFurthest = "このリバースオプションは最も近いポイントと同様に動作し、処理速度もほぼ同じです。コライダー全体が範囲内に収まっていることを確認したい場合に最適な選択です。";
        protected const string TFocused = "このオプションを使えば、フォーカスポイントをカスタムで移動させることができます。特定の地点を検出ポイントに注視させたい場合に使用してください。";    
        protected const string TDodge = "回避オプションはデフォルトで Pivot を基準としており、複数のチェックを通じて障害物を回避する最適なルートを探索します。このオプションは他の方式に比べてやや処理負荷が高いものの、賢い視線シミュレーションを実現します。";
