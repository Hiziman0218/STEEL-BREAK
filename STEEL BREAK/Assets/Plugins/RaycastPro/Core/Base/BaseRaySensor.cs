namespace RaycastPro
{
    ...
    public abstract class BaseRaySensor<R, E, P> : RaycastCore where P : BasePlanar
    {
        ...
        [Tooltip("レイの軌道に沿って描画される Line Renderer")]
        public LineRenderer liner;

        [Tooltip("true の場合、Line Renderer の終点位置を手動で設定できます")]
        public bool linerClamped;
        
        [Tooltip("ヒット位置でラインをカットします。")]
        public bool linerCutOnHit;
        
        [Tooltip("このプロジェクトでは、LineRenderer をレイの進行方向に沿ってカットする処理が行われます。（この機能は IRadius レイで動作します。半径のないレイや拡張レイを使用している場合は無効にしてください）")]
        ...
        [Tooltip("レイ本体上の Transform を制御するためのハンドラーです。")]
        public Transform stamp;

        [Tooltip("レイのローカル座標系での発射方向")]
