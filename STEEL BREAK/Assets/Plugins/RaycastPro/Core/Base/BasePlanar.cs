...
namespace RaycastPro
{
    ...
    public abstract class BasePlanar : RaycastCore
    {
        ...
        [Header("無効化されたクローンは、RayCast を継続しなくなります。このオプションは Line Renderer には影響しません（CutOnHit を除く）ので、クローン処理の最適化と安全性向上に役立ちます。"), Tooltip("無効化されたクローンは、RayCast を継続しなくなります。このオプションは Line Renderer には影響しません（CutOnHit を除く）ので、クローン処理の最適化と安全性向上に役立ちます。")]
