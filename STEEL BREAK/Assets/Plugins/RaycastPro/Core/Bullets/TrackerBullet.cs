namespace RaycastPro.Bullets
{
    ...
    public sealed class TrackerBullet : Bullet
    {
        ...
        protected override void OnCast()
        {
            ...
            if (!target)
            {
                RCProEditor.Log($"<color=#4AFF98>{caster.name}</color> ［消失!!］ <color=#FF1E21>TrackTarget</color> transform!");
            }
#endif
...
#if UNITY_EDITOR
        internal override string Info => "発砲者によって発射された後、ターゲットを自動追尾します。" + HAccurate + HDependent;
