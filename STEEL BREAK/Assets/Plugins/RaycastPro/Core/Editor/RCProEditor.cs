...
namespace RaycastPro.Editor
{
    ...
    public sealed class RCProEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            HeaderField(target.GetType().Name.ToRegex() + (_isMulti ? " (マルチ 編集)" : ""));
        ...
        private const string CNoInfoDefinition = "この項目には説明・ヘルプ情報が未定義です。";
        ...
        internal static void ArrayButtonField<T>(ref T[] type, string label = "", string tooltip = "", bool copyLast = false)
        {
            ...
            if (GUILayout.Button("-".ToContent("解除"), GUILayout.Width(30)))
            {
            ...
            if (GUILayout.Button("C".ToContent("クリア"), GUILayout.Width(30)))
            {
                type = new T[] {};
            }
