...
namespace RaycastPro.Editor
{
    ...
    public sealed class RCProPanel : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button($"{heart} レビューを投稿していただきありがとうございます。 {heart}", buttonStyle)) Application.OpenURL("https://assetstore.unity.com/packages/tools/physics/raycastpro-214714#reviews");
            if (GUILayout.Button($"チュートリアル動画  {play} YouTube", buttonStyle)) Application.OpenURL("https://www.youtube.com/@KiynL");
            ...
            GUILayout.Space(2);
            RCProEditor.GUILine(lineColor);
            showOnStart = EditorGUILayout.Toggle("開始時にパネルを表示する", showOnStart);
            RCProEditor.GUILine(lineColor);
        }
        ...
        public static void Init()
        {
            ...
            window = GetWindow(typeof(RCProPanel), true, "RaycastPro パネル", true);
        ...
        {
            RCProEditor.Log("<color=#00FF00>設定の保存</color>");
        ...
        {
            if (message) RCProEditor.Log("<color=#00FF00>設定が読み込まれました.</color>");
