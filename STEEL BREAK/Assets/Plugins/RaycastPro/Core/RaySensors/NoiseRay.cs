...
namespace RaycastPro.RaySensors
{
    ...
    public sealed class NoiseRay : RaySensor, IPulse
    {
        ...
        [Tooltip("レイを発射する前に方向が変化（パルス）するため、この機能を使用する場合はパルスタイムをオフにすることを推奨します。")]
        public bool pulseWhenCast;
        public bool onUnitCircle;
        
        public Vector2 noiseRange = new Vector2(.2f, .2f);

        [Tooltip("レイの方向を自動的に変更する更新間隔（秒）を指定します。")]
...
#if UNITY_EDITOR
        internal override string Info => "指定された楕円エリアの周囲に、一定間隔でレイをランダムに発射します。" + HIPulse + HAccurate + HDirectional;
            ...
            bool hasInfo = true)
        {
            if (hasMain)
            {
                ...
                if (GUILayout.Button("設定する", GUILayout.Width(60))) OnPulse();
                EndHorizontal();
