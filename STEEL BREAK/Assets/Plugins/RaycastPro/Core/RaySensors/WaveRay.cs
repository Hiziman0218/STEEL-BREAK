...
namespace RaycastPro.RaySensors
{
    ...
    public sealed class WaveRay : PathRay, IRadius
    {
        ...
        [Tooltip("パルスの間隔（または量）を秒単位で指定します。")]
        public float waveSpeed = 1f;
        [Tooltip("パワーの傾斜に基づいた、波の湾曲量です。\\n\\\n[0: 破綻した波] \\n\\\n[1: 通常の波] \\n\\\n[2: 調和的で滑らかな波]")]
        public float power = 1;
        [Tooltip("秒単位でのパルス間隔です。")]
...
#if UNITY_EDITOR
        internal override string Info => "サイン関数（Sinusカーネル）を用いた数学的関数に基づいてレイを送信し、ヒット情報を取得します。" + HAccurate+HDirectional+HPathRay+HIRadius;
            ...
            bool hasEvents = true, bool hasInfo = true)
        {
            if (hasMain)
            {
                ...
                PropertySliderField(_so.FindProperty(nameof(offsetX)), 0f, Mathf.PI * 2f, "OffsetX".ToContent("WaveのX方向オフセットは以下の単位で指定します。 \"「ラジアン」\"."));
