namespace RaycastPro.RaySensors
{
    ...
    public sealed class PointerRay : RaySensor, IRadius
    {
        ...
        [Tooltip("レイの起点がカメラ上に直接配置されます。")]
        public bool cameraBase = false;
        
        [Tooltip("有効にすると、最初のレイキャストはカメラから発射されてマウス位置を検出し、2本目のレイはコンポーネントの持ち主（オブジェクト）から直接発射されます。")]
...
#if UNITY_EDITOR
        internal override string Info => "指定したオブジェクトからレイを発射できるマウス位置トラッカーで、この機能を即座に起動するために使われます。" + HAccurate+HIRadius+HDependent;
