namespace RaycastPro.Detectors
{
    ...
    {
        [Tooltip("指定されたレイヤー上のサウンドと衝突させ、専用のコライダーディテクターを割り当てることで、自動的にサウンドをフィルタリングソースへ送信します。")]
...
#if UNITY_EDITOR
       internal override string Info => "アバターの構成情報を検出し、現在の視認率（カメラに映っている割合）をフィードバックします。\n" + HAccurate + HIPulse+HDependent+HPreview;
