...
namespace RaycastPro.Detectors
{
    ...
    {
        [Tooltip("指定レイヤー上の音源にコライダーを関連付け、専用ディテクターで検出されたサウンドを自動的にフィルタリングソースへ送信します。")]
        public ColliderDetector lightFinder;
        
        [Tooltip("本システムは、ディレクショナルライトおよびポイントライトに対応済みです。")]
...
#if UNITY_EDITOR
        internal override string Info => "指定された光源を基準に、環境内に散乱するレイキャストを用いて、光の色と輝度を検出する処理です。\n" + HDependent +HIPulse;
