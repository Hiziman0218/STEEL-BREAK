...
namespace RaycastPro.Detectors
{
    ...
    {
        [Tooltip("指定されたレイヤーのサウンドと物理コライダーを連携させ、専用の検出システム（Collider Detector）を使って、それらの音を自動的にフィルター処理に送ります。")]
        ...
        [Tooltip("聴力（Hearing Power）の値が高いキャラクターほど、壁の遮音効果や距離による音の減衰が軽減されます。")]
        public float hearingPower = 50;
        [Tooltip("")]
        public int sampleWindow = 64;

        [Tooltip("この値以上の音量でなければ検出されません（調整しやすいように100倍スケーリングされています）。")]
        public float loudnessThreshold;

        [Tooltip("有効にすると、実際の音量に基づいて音の検出が行われます。無効時は範囲内に入るだけで常に最大音量として処理されます。")]
        public bool affectSoundLoudness = true;
        
        [Tooltip("壁の厚さは聴取レベルに影響し、壁が厚くなるほど、壁の裏からの音を聞き取る力が低下します。")]
...
#if UNITY_EDITOR
        internal override string Info => "レイキャストを使って壁や障害物の有無をチェックし、それによって音の通過を制限（減衰）する処理を行います。\nこれにより、現実に近い音響遮断効果が実現されます。\n" + HAccurate+HIPulse+HDependent;
