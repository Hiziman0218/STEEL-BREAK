...
{
    /// <summary>
    /// 【EmeraldAnimatorGenerator】
    /// アニメーションプロファイルの複製と、Animator Controller（コントローラ）の自動生成・反映を行うエディタ拡張ユーティリティ。
    /// ・選択した AnimationProfile の「一意なコピー」を作成し、コピー側の Animator 設定をクリア
    /// ・各ステート名に基づいて、指定の AnimationProfile のアニメーションや速度・ミラー設定を Animator Controller に適用
    /// </summary>
    public class EmeraldAnimatorGenerator
    {
        /// <summary>
        /// 渡された AnimationProfile の「一意なコピー」を作成し、コピー側の Animator Controller 設定をクリアします。
        /// </summary>
        public static void CopyAnimationProfile(AnimationProfile AnimationProfileRef)
        {
            ...
            // コピー側の Animator Controller をクリア
        ...
        /// <summary>
        /// 渡された AnimationProfile（EmeraldAnimation の値群）を用いて Animator Controller を生成・更新します。
        /// ステート名を探索し、各ステートへ適切なアニメーション・再生速度・ミラー設定を適用します。
        /// </summary>
        ...
        /// <summary>
        /// 指定のサブステート（名前一致）に対し、渡された AnimationClass リストの内容を各ステートへ順番に適用します。
        /// </summary>
        static void SetState(ChildAnimatorStateMachine childAnimatorStateMachine, string stateName, ChildAnimatorState[] childAnimatorState, List<AnimationClass> animationList)
        {
            if (childAnimatorStateMachine.stateMachine.name == stateName)
            {
                ...
                // 渡されたリスト数より多いステートは、不要のため Motion を null にしてクリア
        ...
        static void ApplyNonCombatAnimations(AnimationProfile EmeraldAnimationComponent, AnimatorController m_AnimatorController)
        {
            ...
            for (int i = 0; i < m_AnimatorController.layers[0].stateMachine.stateMachines.Length; i++)
            {
                ...
                if (m_AnimatorController.layers[0].stateMachine.stateMachines[i].stateMachine.name == "Emote States")
                {
                    for (int f = 0; f < EmeraldAnimationComponent.EmoteAnimationList.Count; f++)
                    {
                        ...
                        //m_AnimatorController.layers[0].stateMachine.stateMachines[i].stateMachine.states[f].state.speed = EmeraldAnimationComponent.EmoteAnimationList[f].AnimationSpeed; //TODO: 速度パラメータの対応が必要
                    }
                }
            }

            // 個別ステート名で探索し、該当のモーション/速度/ミラー設定を適用
            ...
            // Movement Blend Tree（移動ブレンドツリー）の取得と適用
            ...
            // Idle Blend Tree へアニメーションと設定を適用
            ...
            // Walk Blend Tree へアニメーションと設定を適用（左・前・右）
            ...
            // 非戦闘移動のしきい値（Animator Type に応じて調整）
            ...
            // Run Blend Tree へアニメーションと設定を適用（左・前・右）
        ...
        /// <summary>
        /// 武器タイプ1（Type 1）のアニメーションを Animator Controller に適用します。
        /// </summary>
        static void ApplyType1Animations(AnimationProfile EmeraldAnimationComponent, AnimatorController m_AnimatorController)
        {
            ...
            // Combat Movement Blend Tree（戦闘時移動ブレンドツリー）の取得と適用
            ...
            // Combat Idle Blend Tree へアニメーションと設定を適用
            ...
            // Combat Walk Blend Tree へアニメーションと設定を適用（左・前・右）
            ...
            // 戦闘時移動のしきい値（Animator Type に応じて調整）
            ...
            // Combat Run Blend Tree へアニメーションと設定を適用（左・前・右）
        ...
        /// <summary>
        /// 武器タイプ2（Type 2）のアニメーションを Animator Controller に適用します。
        /// </summary>
