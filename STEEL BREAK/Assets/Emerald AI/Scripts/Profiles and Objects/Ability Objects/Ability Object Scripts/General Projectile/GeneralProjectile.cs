...
namespace EmeraldAI
{
    ...
    {
        #region 変数
        [Header("現在のアビリティデータ（GeneralProjectileAbility）")]
        ...
        void Impact(GameObject TargetHit)
        {
            ...
            {
                if (!CurrentAbilityData.GeneralProjectileSettings.AttachToTarget) SetEffectsState(false); // EffectsToDisable の名前と一致するエフェクトを無効化
