...
public class Weapon_Backpack : MonoBehaviour , IWeapon
{
    [Header("設定")]
    ...
    private string m_checkPoint = "AttachPoint"; //装備するときに探索するポイント
    ...
    public void AttachToPoint(Transform point, AttachSide side)
    {
        ...
        //AttachPointのローカル回転を打ち消す(★重要★)
        //AttachPointのローカル回転を逆にかけることで、AttachPointの向きを手の向きと一致
        transform.localRotation = Quaternion.Inverse(grip.localRotation);

        //AttachPointのローカル位置を打ち消す
        //AttachPointがPivotからどれだけ離れているか(ローカル位置)を反転して適用することで、
        //AttachPointの位置が手の原点(hand.position)と一致するよう補正
